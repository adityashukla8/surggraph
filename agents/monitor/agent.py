...
def _ensure_phase_node(case_id: str, phase: str, phases_seen: set[str], time_range_label: str) -> None:
...
def monitor_case(case_id: str, video_id: str, start_s: float = 0.0, end_s: float | None = None) -> list[DivergenceEvent]:
    ...
    async def on_window_complete(assessment: MonitorWindowAssessment) -> None:
        ...
        _ensure_phase_node(case_id, phase, phases_seen, phase_time_range)
    ...
    asyncio.run(run_monitor_sweep(video_id, start_s=start_s, end_s=end_s, on_window_complete=on_window_complete))
    return events
